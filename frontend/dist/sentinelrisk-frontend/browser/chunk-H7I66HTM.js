...
var KeycloakService2 = class _KeycloakService {
  constructor(keycloakAngular) {
    this.keycloakAngular = keycloakAngular;
  }
  init() {
  ...
  getUserRoles() {
    ...
    console.debug(`\u{1F50D} [KeycloakService] getUserRoles() -> R\xF4les bruts:`, roles);
    return roles;
  }
  hasRole(role) {
    console.group(`\u{1F512} [KeycloakService] V\xE9rification du r\xF4le: "${role}"`);
    ...
    console.debug(`\u{1F464} R\xF4les utilisateur:`, userRoles);
    if (!userRoles || userRoles.length === 0) {
      console.warn("\u274C Aucun r\xF4le trouv\xE9 pour l'utilisateur");
      console.groupEnd();
      return false;
    }
    const normalizedRole = role.toLowerCase().replace(/^role_/, "");
    console.debug(`\u{1F504} R\xF4le normalis\xE9 recherch\xE9: "${normalizedRole}"`);
    const hasRole = userRoles.some((userRole) => {
      ...
      if (match) {
        console.debug(`\u2705 Match trouv\xE9: "${userRole}" -> "${normalizedUserRole}" === "${normalizedRole}"`);
      }
      return match;
    });
    console.debug(`\u{1F3AF} R\xE9sultat final: ${hasRole ? "\u2705 AUTORIS\xC9" : "\u274C REFUS\xC9"}`);
    console.groupEnd();
    return hasRole;
  }
  hasAnyRole(roles) {
    console.group(`\u{1F512} [KeycloakService] V\xE9rification de r\xF4les multiples:`, roles);
    const result = roles.some((role) => {
      const hasRole = this.hasRole(role);
      console.debug(`- ${role}: ${hasRole ? "\u2705" : "\u274C"}`);
      return hasRole;
    });
    console.debug(`\u{1F3AF} R\xE9sultat final hasAnyRole: ${result ? "\u2705 AUTORIS\xC9" : "\u274C REFUS\xC9"}`);
    console.groupEnd();
    return result;
  }
  getUsername() {
    const userDetails = this.keycloakAngular.getKeycloakInstance().tokenParsed;
    return userDetails ? userDetails["preferred_username"] : void 0;
  }
  /**
  ...
  refreshUserInfo() {
    return __async(this, null, function* () {
      try {
        console.log("[KeycloakService] Rafra\xEEchissement des informations utilisateur...");
        yield this.keycloakAngular.getKeycloakInstance().updateToken(5);
        console.log("[KeycloakService] Token rafra\xEEchi avec succ\xE8s");
        this.logUserRoles();
      } catch (error) {
        console.error("[KeycloakService] Erreur lors du rafra\xEEchissement du token:", error);
      }
    });
  }
  /**
  ...
  */
  isTokenExpiring(minValidity = 30) {
  ...
  logUserRoles() {
    console.group("\u{1F464} Informations utilisateur Keycloak");
    console.log("Est connect\xE9:", this.isLoggedIn());
    console.log("R\xF4les utilisateur:", this.getUserRoles());
    const standardRoles = ["admin", "risk_manager", "compliance_officer", "auditor", "user"];
    standardRoles.forEach((role) => {
      console.log(`A le r\xF4le '${role}': ${this.hasRole(role)}`);
    });
    const token = this.keycloakAngular.getKeycloakInstance().tokenParsed;
    if (token) {
      console.log("D\xE9tails du token:", {
        sub: token["sub"],
  ...
  static {
    this.\u0275fac = function KeycloakService_Factory(t) {
      return new (t || _KeycloakService)(\u0275\u0275inject(KeycloakService));
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _KeycloakService, factory: _KeycloakService.\u0275fac, providedIn: "root" });
  }
};
...
js-sha256/src/sha256.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-H7I66HTM.js.map
